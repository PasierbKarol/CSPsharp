...
namespace CSPlang
{
	...
	*/
    ...
    {
        /** The monitor synchronizing the writers and alting reader */
        //Apparently this has to be Monitor class to have required functionality: https://stackoverflow.com/questions/209281/c-sharp-equivalent-to-javas-wait-and-notify 
		 ...
		 */
        public Alternative(Guard[] guard) //Guard here was readonly in JCSP
        {
            this.guard = guard;
        ...
        {
            //TODO see why this code was commented and how it looks like in JCSP
            // if (barrierPresent) {
        ...
        public /*readonly*/ int fairSelect()
        {
            ...
            lock (altMonitor)
            {
                if (state == enabling)
                {
                    ...
                    try
                    {
                        //TODO check this code in JCSP and decide what to do with it
                        //if (timeout)
                        //{
        ...
        private /*readonly*/ void enableGuards()
        {
            if (barrierPresent)
            {
                // Debug.WriteLine("ENABLE barrier(s) present ...", "DEBUG");
            ...
            for (enableIndex = favourite; enableIndex < guard.Length; enableIndex++)
            {
                ...
                {
                    //TODO decide what to do with that code
                    ...
                    {
                        //Debug.WriteLine("ENABLE " + enableIndex + " NON-BARRIER SUCCEED", "DEBUG");
        ...
        private void disableGuards()
        {
            if (selected != favourite)
            {
                ...
                {
                    //TODO see if that is needed and whether can be removed
            ...
            lock (altMonitor)
            {
                if (state == enabling)
                {
                    ...
                    try
                    {
                        if (timeout)
                        {
                            long delay = msecs - CSTimer.CurrentTimeMillis();
                            ...
                            if (delay > Spurious.earlyTimeout)
                            {
                                ...
                                while (state == waiting)
                                {
                                    delay = msecs - CSTimer.CurrentTimeMillis();
                        ...
                        {
                            //TODO should that spurious reference be removed?
                            Monitor.Wait(altMonitor);
                            while (state == waiting)
                            {
                                if (Spurious.logging)
        ...
        private void disableGuards(Boolean[] preCondition)
        {
            if (selected != favourite)
            {
                ...
                if (selected == NONE_SELECTED)
                {
                    //TODO just as the same above. Is it needed?
                    // System.out.println ("disableGuards: NONE_SELECTED ==> " + timeIndex);
                    // NOTE: this is a work-around for Java wait-with-timeouts sometimes
                    // returning early.  If this did not happen, we would not get here!
                    selected = timeIndex;
