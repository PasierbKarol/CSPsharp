...
namespace CSPlang
{
     ...
     * with a single word change (replace <TT>Sequence</TT> by <TT>CSPParallel</TT>)
     ...
     * invoked.
     * </P>
     * <H2>Example - Matrix Multiply (in Sequence and in CSPParallel)</H2>
     * <P>
     ...
     *
     * <H3><A NAME="parMultiply">CSPParallel Implementation</H3>
     ...
     *     }
     * <I></I>
     *     new CSPParallel (rowProcess).run ();
     * <I></I>
     *   }
     * </PRE>
     ...
     *
     * <H3><TT>Sequence</TT> versus <TT>CSPParallel</TT></H3>
     * Beware that it is only in certain circumstances that <TT>Sequence</TT> and <TT>CSPParallel</TT>
     ...
     * The first time it is <TT>run</TT>, the JCSP {@link jcsp.lang.CSPParallel} process
     ...
     * When (and if) all those component processes terminate, the <TT>CSPParallel</TT>
     * <TT>run</TT> terminates but leaves the newly created threads parked for later use.
     * Subsequent <TT>run</TT>s of the <TT>CSPParallel</TT> process resuse those parked threads.
     * So the overhead of thread creation occurs only for the <I>first</I> run.
     * <P>
     * In <A HREF="#parMultiply"><TT>parMultiply</TT></A>, however, the <TT>CSPParallel</TT>
     ...
     * <P>
     * There are two ways to fix this.  The first is to accept that the <TT>CSPParallel</TT>
     ...
     * invoking {@link jcsp.lang.CSPParallel#releaseAllThreads <TT>releaseAllThreads</TT>} on
     * the <TT>CSPParallel</TT> after it has been <TT>run</TT> - the memory associated with
     ...
     * <PRE>
     *     final CSPParallel par = new CSPParallel (rowProcess);
     *     par.run ();
     *     par.releaseAllThreads ();
     * </PRE>
     *
     * The second way to improve things is to save the <TT>CSPParallel</TT> process for later
     ...
     * invoked to see if a suitable <TT>CSPParallel</TT> process had already been saved.
     * <P>
     ...
     * in - the <TT>CSPParallel</TT> process.  So, we change the method into something that
     ...
     * <PRE>
     *   public static CSPParallel makeParMultiply (final double[][] X,
     *                                           final double[][] Y,
     *                                           final double[][] Z) {
     * <I></I>
     ...
     *     }                                                              // as before
     * <I></I>
     *     return new CSPParallel (rowProcess);
     * <I></I>
     *   }
     * </PRE>
     ...
     * <PRE>
     *     final CSPParallel par = Matrix.makeParMultiply (X, Y, Z);
     * <I></I>
     *     while (...) {
